...
void Input(char matrix[][WIDTH], int n, int m) {  // shereeti
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
...
void Output(char matrix[][WIDTH], int n, int m) {  // shereeti
    printw("\n");
...
int SpeedChange(char button, int pause) {  // daliatoo
    if (button == '+')
        pause -= 20000;
    else if (button == '-')
        pause += 20000;
...
void NewGen(char A[][WIDTH], char B[][WIDTH], int n, int m) {  // chickpet
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) {
...
int CountNeighbours(char matrix[][WIDTH], int y, int x) {  // chickpet
    int res = 0;
    int coor_x, coor_y;
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
...
int isTheSame(char A[][WIDTH], char B[][WIDTH], int n, int m) {  // chickpet
    int flag = 1;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++)
...
void Swap(char A[][WIDTH], char B[][WIDTH], int n, int m) {  // daliatoo
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) {
            char temp = A[i][j];
            A[i][j] = B[i][j];
